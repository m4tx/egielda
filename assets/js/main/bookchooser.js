...
/**
 * Book chooser used in Book Chooser Wizard.
 */
...
egielda.bookChooser = function() {
    ...
    $('button[name="btn-next"],button[name="btn-back"]').on('click', function() {
        $('form').append($('<input type="hidden" name="book_data">')
            .attr('value', JSON.stringify(chosenBooks))).submit();
    });
    ...
    function addBookTr(tr) {
        var button = $('<button class="ui link button btn-remove-book">' +
            '<i class="icon trash"></i>' + gettext('Remove') + '</button>');
